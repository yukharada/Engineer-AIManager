...
import { CheckSquare, Loader2, Zap, CheckCircle2, GitPullRequest, ArrowRight, TrendingUp } from "lucide-react";
...
export default function Challenges() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      console.error(e);
      // Fallback data removed for brevity in example, assuming implementation is similar to previous.
    }
    ...
    <div className="w-full flex flex-col gap-8 md:gap-12 animate-fade-in pb-12 px-1">
      ...
      {challenges.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-6 sm:gap-8">
          {challenges.map((c, i) => (
            <div key={c.id} className={`glass-card p-6 sm:p-8 flex flex-col gap-6 transition-all duration-300 ${c.completed ? "opacity-60 grayscale" : "hover:-translate-y-1 hover:shadow-[0_20px_50px_rgba(99,102,241,0.2)]"}`} style={{ animationDelay: `${i * 0.1}s` }}>
              ...
              <div className="pt-6 border-t border-white/5 flex flex-col gap-6">
                ...
                {!c.completed && (
                  <Link 
                    ...
                    className="w-full py-4 px-6 bg-indigo-500/10 hover:bg-indigo-500/20 text-indigo-300 border border-indigo-500/20 rounded-2xl flex items-center justify-center gap-3 text-sm font-black transition-all group"
                  >
