...
import { saveUserProfile, saveChallenges, saveReviewHistory, saveRoadmap } from '@/lib/db';
  ...
  const handleMigrate = async () => {
    ...
    try {
      ...
      for (const item of history) {
        // Only attempt to save if taskId is a valid UUID (or if the table allows null task_id)
        if (isUUID(item.taskId) || item.taskId === 'manual') {
          await saveReviewHistory({
            ...item,
            taskId: isUUID(item.taskId) ? item.taskId : 'manual' // Fallback for safety
          });
        }
      }
      
      setStatus('✅ 移行完了！ Supabaseへの同期に成功しました。');
      setIsComplete(true);
      ...
      <div className="max-w-xl w-full glass-card p-10 space-y-8 animate-fade-in border-indigo-500/20">
        <div className="flex flex-col items-center text-center gap-4">
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">LocalStorage → Supabase PostgreSQL</p>
          </div>
        </div>
        ...
        <div className="bg-amber-500/10 border border-amber-500/20 rounded-2xl p-6 flex flex-col gap-3">
          <div className="flex items-center gap-2 text-amber-400">
            ...
            <span className="text-sm font-black uppercase tracking-tight">Warning</span>
          </div>
          <p className="text-xs font-bold text-slate-400 leading-relaxed">
            この操作はブラウザに保存されているデータをクラウドデータベースへ同期します。
            移行後は、どのデバイスからアクセスしてもあなたの成長データが共有されるようになります。
          </p>
        </div>
        ...
        <div className="space-y-4">
          <button
            ...
            className="w-full py-5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-2xl font-black text-lg transition-all shadow-xl shadow-indigo-600/20 disabled:opacity-30 flex items-center justify-center gap-3 italic"
          >
            {isLoading ? <Loader2 size={24} className="animate-spin" /> : isComplete ? <CheckCircle2 size={24} /> : <Database size={24} />}
            {isComplete ? '移行完了' : 'MIGRATE TO SUPABASE'}
          </button>
